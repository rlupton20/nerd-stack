...
impl VirtType {
    ...
    unsafe fn perform_ioctl(&self, fd : c_int, ifreq : IfReq)
        ...
        if rc < 0 {
            ...
            match *self {
                VirtType::TUN => Ok(VirtDevice::TUN { fd : fd, }),
                VirtType::TAP => Ok(VirtDevice::TAP { fd : fd, })
            }
        }
    }
        

    pub fn open(&self, path : &str) -> Result<VirtDevice, &'static str> {
